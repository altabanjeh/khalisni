  ...
  if (!items.length) {
    return (
      <div className={isPublic
        ? 'rounded-lg border border-dashed border-[var(--khalsni-public-border)] bg-white/10 px-5 py-6 text-sm font-semibold leading-7 text-white/50'
        ...
        return (
          <li key={item.id || index} className="relative grid grid-cols-[44px_minmax(0,1fr)] gap-4 pb-6 last:pb-0">
            <div className="relative flex justify-center">
              {!isLast ? <span className={isPublic ? 'absolute top-11 h-[calc(100%-1.5rem)] w-px bg-white/20' : 'absolute top-11 h-[calc(100%-1.5rem)] w-px bg-border'} /> : null}
              <span className={isLast
                ...
                : isPublic
                  ? 'relative z-10 flex h-10 w-10 items-center justify-center rounded-full bg-green-400/20 text-green-100 ring-1 ring-green-300/25'
            ...
            <article className={isPublic
              ? 'rounded-lg border border-white/10 bg-white/10 p-4'
              : isLast
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className={isPublic ? 'text-xs font-semibold text-white/50' : 'text-xs font-semibold text-slate-500'}>{formatDateTime(item.created_at, language)}</span>
              </div>
              <p className={isPublic ? 'mt-3 text-sm font-semibold leading-7 text-white/50' : 'mt-3 text-sm leading-7 text-slate-600'}>
