  ...
  return (
    <PublicPageShell>
      ...
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <PublicPanel>
          {registered ? (
            <div className="mb-4 rounded-[var(--radius-lg)] border border-green-200 bg-green-50 px-4 py-3 text-sm font-semibold text-green-700">
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            <div>
              <label htmlFor="login-email" className="mb-2 block text-sm font-bold text-ink">{isArabic ? 'البريد الإلكتروني' : 'Email'}</label>
              <PublicInput id="login-email" autoComplete="email" type="email" {...register('email')} />
            </div>
            <div>
              <label htmlFor="login-password" className="mb-2 block text-sm font-bold text-ink">{isArabic ? 'كلمة المرور' : 'Password'}</label>
              <PublicInput id="login-password" autoComplete="current-password" type="password" {...register('password')} />
            </div>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <label className="flex items-center gap-3 text-sm font-semibold text-slate-600" htmlFor="login-remember">
                <input id="login-remember" className="h-4 w-4 rounded border-[var(--khalsni-public-border)] accent-[var(--khalsni-public-primary)]" type="checkbox" {...register('remember')} />
        ...
        <PublicCard className="flex flex-col justify-between">
          <div>
            ...
            <h2 className="mt-4 text-xl font-extrabold text-ink">{isArabic ? 'ليس لديك حساب؟' : "Don't have an account?"}</h2>
          ...
          <Link
            className="mt-6 inline-flex h-11 items-center justify-center rounded-[var(--radius-md)] border border-[var(--khalsni-public-border)] bg-white px-5 text-sm font-extrabold text-ink transition hover:bg-[var(--khalsni-public-bg-secondary)]"
