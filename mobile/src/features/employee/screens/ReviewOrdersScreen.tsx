...
export function ReviewOrdersScreen({ navigation }: { navigation: any }) {
  const query = useQuery({ queryKey: ['employee-orders'], queryFn: () => ordersApi.getEmployeeOrders() });

  return (
    <AppScreen title="مراجعة الطلبات" refreshing={query.isRefetching} onRefresh={query.refetch}>
      {query.isLoading ? <LoadingState /> : null}
      {query.isError ? <ErrorState message={getDisplayError(query.error)} onRetry={query.refetch} /> : null}
      {!query.isLoading && !query.isError && !(query.data?.length) ? <EmptyState title="لا توجد طلبات للمراجعة" /> : null}
      <FlatList
        data={query.data ?? []}
        scrollEnabled={false}
